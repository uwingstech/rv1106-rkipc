...
typedef enum __rtsp_msg_auth_type_e
{
	RTSP_MSG_AUTH_TYPE_BASIC = 0,
	RTSP_MSG_AUTH_TYPE_DIGEST,
	RTSP_MSG_AUTH_TYPE_BUTT,
} rtsp_msg_auth_type_e;

//Authorization R opt. all
typedef struct __rtsp_msg_auth_s
{
	rtsp_msg_auth_type_e type;
	char basic_b64[64];
} rtsp_msg_auth_s;

//WWW-Authenticate r opt. all
typedef struct __rtsp_msg_www_auth_s
{
	rtsp_msg_auth_type_e type;
	char realm[64];
} rtsp_msg_www_auth_s;
...
//x-Accept-Dynamic-Rate r opt. DESCRIBE
typedef struct __rtsp_msg_x_accept_dynamic_rate
{
	uint32_t x_accept_dynamic_rate;
} rtsp_msg_x_accept_dynamic_rate;

//for QuickTime Player
//x-Dynamic-Rate Rr opt. SETUP
typedef struct __rtsp_msg_x_dynamic_rate
{
	uint32_t x_dynamic_rate;
} rtsp_msg_x_dynamic_rate;
...
typedef struct __rtsp_msg_hdr_s
{
	...
	rtsp_msg_x_dynamic_rate *x_dynamic_rate; //for QuickTime Player

	//request-headers
	rtsp_msg_accept_s *accept;
	rtsp_msg_auth_s *auth;
	...
	rtsp_msg_www_auth_s *www_auth;
	rtsp_msg_x_accept_dynamic_rate *x_accept_dynamic_rate; //for QuickTime Player
